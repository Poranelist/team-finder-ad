...
def create_avatar(name, surname, email):
    ...
    color_index = int(hashlib.md5(email.encode()).hexdigest(), 16) % len(
        AVATAR_COLOR_PALETTE
    )

    bg_color = AVATAR_COLOR_PALETTE[color_index]
    ...
    return ContentFile(
        buffer.read(),
        name=f"avatar_{filename_hash}.png",
    )
